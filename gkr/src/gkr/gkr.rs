...
pub fn gkr_prover<F: PrimeField>(circuit: &mut Circuit<F>, input: Vec<F>) -> Proof<F> {
    let mut circuit_evaluation = circuit.evaluate(input);
    ...
    transcript.append(&convert_poly_to_bytes(output_poly.clone().evaluated_value));

    let random_challenge = transcript.sample_field_element();

    let mut claimed_sum = output_poly.evaluate_poly(&[random_challenge]).expect("failed");
...
pub fn verify<F: PrimeField>(
    ...
    let mut claimed_sum = w0_polynomial.evaluate_poly(&vec![random_challenge_a]).expect("failed");
    ...
    for layer_index in 0..circuit.layers.len() {
        ...
        let verify_result = gkr_partial_verify(proof.sumcheck_proofs[layer_index].clone(), &mut transcript);
...
fn get_folded_verifier_claim<F: PrimeField>(
    ...
    let add_r = new_add_i.evaluate_poly(&current_random_challenge.to_vec()).expect("failed");
    let mul_r = new_mul_i.evaluate_poly(&current_random_challenge.to_vec()).expect("failed");
...
pub fn evaluate_wb_wc<F: PrimeField>(
    ...
    let wb_poly_evaluated = wb_poly.clone().evaluate_poly(rb_values).expect("failed");
    let wc_poly_evaluated = wc_poly.clone().evaluate_poly(rc_values).expect("failed");

    (wb_poly_evaluated, wc_poly_evaluated)
}

fn convert_poly_to_bytes<F: PrimeField>(poly: Vec<F>) -> Vec<u8> {
    poly.iter()
        .flat_map(|f| f.into_bigint().to_bytes_be())
        .collect::<Vec<_>>()
}
...
mod tests {
    ...
    fn test_f_b_c_poly() {
        ...
        let add = f_b_c_poly(1, layer2, &circuit, Fq::from(0));
    }
    ...
    fn test_tensor_addition() {
        ...
        let poly_1 = MultilinearPoly::new(b.clone(), b.len().ilog2() as usize).expect(
            "failed to identify poly"
        );
        let poly_2 = MultilinearPoly::new(c.clone(), c.len().ilog2() as usize).expect(
            "failed to identify poly"
        );
        let result = MultilinearPoly::new(d.clone(), d.len().ilog2() as usize).expect(
            "failed to identify poly"
        );
